...
const App = () => {
  ...
  return (
    <div>
      <h1>Chirag's Death Note </h1>
      <input type="text" value={name} onChange={(e) => setName(e.target.value)} />
      <button onClick={handleSetName}>Write In Note</button><br></br><br></br>
      <input type="text" value={deleteName} onChange={(e) => setDeleteName(e.target.value)} />
      <button onClick={handleDeleteName}>Delete From Note</button><br></br><br></br>
      <button onClick={handleGetNames}>Send Name's to Reaper</button>
      <button onClick={handleResetNames}>Erase Every  Person</button>
      <ul>
